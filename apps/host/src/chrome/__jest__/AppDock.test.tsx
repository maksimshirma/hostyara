...
describe("AppDock", () => {
  ...
  it("renders a real link per app with an href", () => {
    render(<AppDock apps={apps} activeAppId={null} onSelect={() => {}} />);

    const link = screen.getByRole("link", { name: "Рецепты" });
    expect(link).toHaveAttribute("href", "/a/recipes");
  });

  it("marks the active app via aria-current", () => {
    render(<AppDock apps={apps} activeAppId="budget" onSelect={() => {}} />);
  ...
  it("intercepts the click instead of navigating and calls onSelect with the manifest", async () => {
    const onSelect = jest.fn();
    render(<AppDock apps={apps} activeAppId={null} onSelect={onSelect} />);

    await userEvent.click(screen.getByRole("link", { name: "Рецепты" }));

    expect(onSelect).toHaveBeenCalledWith(apps[0]);
    expect(window.location.pathname).not.toBe("/a/recipes");
  });
});
